---
title: "什么是 DNS？"
keywords: "DNS, 域名系统, 域名解析, DNS服务器, 递归查询, 权威DNS"
description: "本文通俗易懂地介绍域名系统（DNS）的核心概念与基本原理。阐述 DNS 作为互联网分布式寻址系统的角色，解析域名与 IP 地址之间的映射关系，并介绍根域名服务器、顶级域服务器及权威服务器的层次化架构，帮助读者理解网络访问基础环节的寻址逻辑与底层通信机制。"
pubDate: 2026-08-15
author: "好机场编辑部"
category: "常用网络工具"
...
- **顶级域名（TLD - Top-Level Domain）**：如通用顶级域 `.com`、`.net`、`.org`，以及国家/地区顶级域 `.cn`、`.jp`、`.uk`。
...
2. **根域名服务器（Root Nameserver）**：全球一共有 13 组根域名服务器逻辑标识（通过 Anycast 技术分布在全球数百个物理站点）。它不存具体 IP，但它清楚知道所有的顶级域名（如 `.com`）该去哪里找。
3. **顶级域名服务器（TLD Nameserver）**：管理特定后缀的所有记录，比如负责告诉查询者 `.com` 旗下的 `example.com` 应该去哪个权威服务器查询。
...
### 2. DNS 与代理分流规则
在使用 Clash、sing-box 等现代网络客户端时，DNS 是核心配置项之一：
